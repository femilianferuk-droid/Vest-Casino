...
def game_bet_keyboard(game: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="10 USDT", callback_data=f"{game}_bet_10", icon_custom_emoji_id=EMOJI["money"]),
         InlineKeyboardButton(text="50 USDT", callback_data=f"{game}_bet_50", icon_custom_emoji_id=EMOJI["money"]),
         InlineKeyboardButton(text="100 USDT", callback_data=f"{game}_bet_100", icon_custom_emoji_id=EMOJI["money"])],
        [InlineKeyboardButton(text="250 USDT", callback_data=f"{game}_bet_250", icon_custom_emoji_id=EMOJI["money"]),
         InlineKeyboardButton(text="500 USDT", callback_data=f"{game}_bet_500", icon_custom_emoji_id=EMOJI["money"]),
         InlineKeyboardButton(text="1000 USDT", callback_data=f"{game}_bet_1000", icon_custom_emoji_id=EMOJI["money"])],
...
async def deposit_start(message: Message, state: FSMContext):
    await message.answer(
        ...
        f"Введите сумму пополнения в USDT (минимум 0.1 USDT):",
        parse_mode=ParseMode.HTML,
...
async def deposit_amount(message: Message, state: FSMContext):
    try:
        ...
        if amount < 0.1:
            await message.answer(f"{e(EMOJI['cross'])} Минимальная сумма пополнения — 0.1 USDT", parse_mode=ParseMode.HTML)
            return
...
{e(EMOJI['link'])} <b><a href='{pay_url}'>НАЖМИТЕ СЮДА ДЛЯ ОПЛАТЫ</a></b>
...
Через Crypto Bot, минимальная сумма 0.1 USDT.
...
async def admin_broadcast_send(message: Message, state: FSMContext):
    ...
    await message.answer(f"{e(EMOJI['loading'])} <b>Начинаю рассылку...</b>", parse_mode=ParseMode.HTML)
