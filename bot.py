...
def games_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            ...
            icon_custom_emoji_id=EMOJI["games"]
        )],
        [InlineKeyboardButton(
            text="Кубик",
...
def basketball_choice_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="Попадание (1.85x)", callback_data="basketball_hit", icon_custom_emoji_id=EMOJI["basketball"]),
        ],
        [
            InlineKeyboardButton(text="Промах (1.5x)", callback_data="basketball_miss", icon_custom_emoji_id=EMOJI["basketball"]),
        ],
        [
            InlineKeyboardButton(text="Попадание 2 раза (3x)", callback_data="basketball_double", icon_custom_emoji_id=EMOJI["basketball"]),
        ],
        [
...
def football_choice_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="Гол (1.85x)", callback_data="football_goal", icon_custom_emoji_id=EMOJI["football"]),
        ],
        [
            InlineKeyboardButton(text="Промах (1.5x)", callback_data="football_miss", icon_custom_emoji_id=EMOJI["football"]),
        ],
        [
...
{e(EMOJI['basketball'])} <b>Баскетбол</b> — Попадание (1.85x) | Промах (1.5x) | Попадание 2 раза (3x)
{e(EMOJI['football'])} <b>Футбол</b> — Гол (1.85x) | Промах (1.5x)
...
async def games_menu(event: Message | CallbackQuery):
    text = f"""
{e(EMOJI['games'])} <b>ВЫБЕРИТЕ ИГРУ</b>

{e(EMOJI['dice'])} <b>Кубик</b> — меньше (1-3) / больше (4-6) | x1.85
{e(EMOJI['basketball'])} <b>Баскетбол</b> — Попадание (1.85x) | Промах (1.5x) | 2 попадания (3x)
{e(EMOJI['football'])} <b>Футбол</b> — Гол (1.85x) | Промах (1.5x)

Минимальная ставка: <b>0.1 USDT</b>
"""
    if isinstance(event, Message):
        await event.answer(text, parse_mode=ParseMode.HTML, reply_markup=games_menu_keyboard())
    else:
        await event.message.edit_text(text, parse_mode=ParseMode.HTML, reply_markup=games_menu_keyboard())
        await event.answer()
...
async def dice_play(callback: CallbackQuery):
    ...
    if is_win:
        win_amount = bet * 1.85
        update_balance(user_id, win_amount - bet)  # Начисляем только чистый выигрыш
...
{e(EMOJI['money'])} +{win_amount - bet:.2f} USDT (чистый выигрыш)
"""
    else:
...
async def basketball_play(callback: CallbackQuery):
    ...
    choice = callback.data
    is_win = random.random() < 0.2  # 20% шанс победы
    ...
    if choice == "basketball_hit":
        ...
        if is_win:
            ...
            result_desc = "Попадание!"
    else:  # basketball_double
        choice_text = "Попадание 2 раза"
        multiplier = 3
        is_win = random.random() < 0.04  # 4% шанс для двойного (0.2 * 0.2)
        if is_win:
