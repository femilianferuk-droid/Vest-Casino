...
# ВАЖНО: Хендлер для фото должен быть зарегистрирован первым
...
# Хендлер для не-фото сообщений в состоянии ожидания скриншота
...
async def admin_broadcast_send(message: Message, state: FSMContext):
    ...
    success = 0
    failed = 0
    status_msg = await message.answer(f"{e(EMOJI['loading'])} <b>Рассылка...</b>\n\n0/{len(db_users)}", parse_mode=ParseMode.HTML)
