...
PRIVACY_URL = "https://telegra.ph/Politika-konfidecialnosti-04-24"
...
class Database:
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS stats (
                    ...
                    dice_wins INT DEFAULT 0, dice_losses INT DEFAULT 0,
                    basketball_wins INT DEFAULT 0, basketball_losses INT DEFAULT 0,
                    football_wins INT DEFAULT 0, football_losses INT DEFAULT 0,
                    blackjack_wins INT DEFAULT 0, blackjack_losses INT DEFAULT 0,
                    bowling_wins INT DEFAULT 0, bowling_losses INT DEFAULT 0,
                    slots_wins INT DEFAULT 0, slots_losses INT DEFAULT 0,
    ...
    async def get_media(self, section: str):
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow("SELECT * FROM media WHERE section = $1", section)
            if row:
                return {"type": row["type"], "file_id": row["file_id"]}
            return None
    ...
    async def set_media(self, section: str, media_type: str, file_id: str):
        async with self.pool.acquire() as conn:
            if file_id and media_type:
                ...
                await conn.execute(
                    "UPDATE media SET type = NULL, file_id = NULL WHERE section = $1",
                    section
                )
...
async def send_with_media(message_or_callback, section: str, text: str, reply_markup=None, parse_mode=ParseMode.HTML):
    """
    Отправляет сообщение с медиа, если оно есть.
    Для CallbackQuery: если есть медиа, удаляет старое сообщение и отправляет новое с медиа.
    Если медиа нет, просто редактирует сообщение.
    Для Message: отправляет ответ с медиа или текстом.
    """
        ...
        try:
            if media_type == "photo":
                if isinstance(message_or_callback, CallbackQuery):
                    # Удаляем сообщение с кнопками и отправляем новое с фото
            ...
            logger.error(f"Error sending media for section '{section}': {ex}")
            # Fallback: если не удалось отправить медиа, отправляем текст
            if isinstance(message_or_callback, CallbackQuery):
    ...
    # Если медиа нет, просто отправляем/редактируем текст
    if isinstance(message_or_callback, CallbackQuery):
        await message_or_callback.message.edit_text(
            text, parse_mode=parse_mode, reply_markup=reply_markup
        )
...
{e(EMOJI['dice'])} <b>Кубик:</b> {e(EMOJI['check'])} {stats['dice']['wins']} / {e(EMOJI['cross'])} {stats['dice']['losses']}
{e(EMOJI['basketball'])} <b>Баскетбол:</b> {e(EMOJI['check'])} {stats['basketball']['wins']} / {e(EMOJI['cross'])} {stats['basketball']['losses']}
{e(EMOJI['football'])} <b>Футбол:</b> {e(EMOJI['check'])} {stats['football']['wins']} / {e(EMOJI['cross'])} {stats['football']['losses']}
{e(EMOJI['blackjack'])} <b>Блэкджек:</b> {e(EMOJI['check'])} {stats['blackjack']['wins']} / {e(EMOJI['cross'])} {stats['blackjack']['losses']}
{e(EMOJI['bowling'])} <b>Боулинг:</b> {e(EMOJI['check'])} {stats['bowling']['wins']} / {e(EMOJI['cross'])} {stats['bowling']['losses']}
{e(EMOJI['slots'])} <b>Слоты:</b> {e(EMOJI['check'])} {stats['slots']['wins']} / {e(EMOJI['cross'])} {stats['slots']['losses']}
...
@router.message(DepositState.waiting_for_rub_screenshot, F.content_type.in_({"photo"}))
async def deposit_rub_screenshot(message: Message, state: FSMContext):
    logger.info(f"Received photo from user {message.from_user.id} for rub deposit")
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            logger.error(f"Failed to send to admin {admin_id}: {ex}")
...
async def admin_broadcast_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        f"Поддерживается <b>HTML</b> и <tg-emoji emoji-id='{EMOJI['check']}'>✅</tg-emoji> премиум эмодзи.",
        parse_mode=ParseMode.HTML, reply_markup=cancel_keyboard()
    )
...
async def admin_broadcast_send(message: Message, state: FSMContext):
    ...
    status_msg = await message.answer(
        f"{e(EMOJI['loading'])} <b>Рассылка...</b>\n\n0/{len(db_users)}",
        parse_mode=ParseMode.HTML
    )
    
    broadcast_text = message.html_text or message.text
    
    for i, user_id in enumerate(db_users.keys()):
        try:
            await bot.send_message(int(user_id), broadcast_text, parse_mode=ParseMode.HTML)
            success += 1
        except Exception as ex:
            logger.error(f"Broadcast failed for {user_id}: {ex}")
            failed += 1
