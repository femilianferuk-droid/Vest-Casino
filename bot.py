...
def main_menu_keyboard():
    ...
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                KeyboardButton(text="Кубик", icon_custom_emoji_id=EMOJI["dice"]),
                KeyboardButton(text="Баскетбол", icon_custom_emoji_id=EMOJI["basketball"])
            ],
            [KeyboardButton(text="Футбол", icon_custom_emoji_id=EMOJI["football"])],
            [
...
def game_bet_keyboard(game: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            ...
            InlineKeyboardButton(text="100", callback_data=f"{game}_bet_100", icon_custom_emoji_id=EMOJI["money"])
        ],
        [
            InlineKeyboardButton(text="250", callback_data=f"{game}_bet_250", icon_custom_emoji_id=EMOJI["money"]),
            InlineKeyboardButton(text="500", callback_data=f"{game}_bet_500", icon_custom_emoji_id=EMOJI["money"]),
            InlineKeyboardButton(text="1000", callback_data=f"{game}_bet_1000", icon_custom_emoji_id=EMOJI["money"])
        ],
        [
            InlineKeyboardButton(text="Своя сумма", callback_data=f"{game}_bet_custom", icon_custom_emoji_id=EMOJI["edit"])
        ],
        [
            InlineKeyboardButton(text="Назад", callback_data="back_to_menu", icon_custom_emoji_id=EMOJI["back"])
        ]
    ])
...
{e(EMOJI['dice'])} <b>Кубик</b> — x2
{e(EMOJI['basketball'])} <b>Баскетбол</b> — x1.5
{e(EMOJI['football'])} <b>Футбол</b> — x3

{e(EMOJI['wallet'])} Пополнение от 0.1 USDT
{e(EMOJI['settings'])} Админ: /admin
"""
    await message.answer(text, parse_mode=ParseMode.HTML)

# ========== ИГРЫ ==========
@router.message(F.text.in_(["Кубик", "Баскетбол", "Футбол"]))
...
async def handle_custom_bet(message: Message):
    ...
    try:
        ...
        if bet < 1:
            await message.answer(f"{e(EMOJI['cross'])} Минимум 1 USDT", parse_mode=ParseMode.HTML)
            return
...
async def admin_edit_balance_user(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"{e(EMOJI['cross'])} Некорректный ID",
            parse_mode=ParseMode.HTML,
            reply_markup=cancel_keyboard()
        )
        return
    ...
    await message.answer(
        ...
        f"{e(EMOJI['money'])} Введите новый баланс:",
        parse_mode=ParseMode.HTML,
        reply_markup=cancel_keyboard()
    )
