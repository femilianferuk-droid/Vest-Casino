...
def main_menu_keyboard():
    """Главное меню с синей кнопкой Игры (style='primary') и премиум-эмодзи"""
    ...
    builder.row(
        ...
        InlineKeyboardButton(
            ...
            icon_custom_emoji_id=EMOJI["withdraw"]
        )
    )
    builder.row(
        InlineKeyboardButton(
...
def games_menu_keyboard():
    """Меню выбора игр БЕЗ премиум-эмодзи в кнопках игр"""
    builder = InlineKeyboardBuilder()
...
🎲 <b>Кубик</b> — меньше (1-3) / больше (4-6) | x1.85
🏀 <b>Баскетбол</b> — Попадание (1.85x) | Промах (1.5x) | 2 попадания (3x)
⚽ <b>Футбол</b> — Гол (1.85x) | Промах (1.5x)
...
async def menu_withdraw(callback: CallbackQuery, state: FSMContext):
    user = get_user(callback.from_user.id)
    if user['balance'] < 0.5:
        await callback.answer("Минимальная сумма вывода 0.5 USDT", show_alert=True)
        return
